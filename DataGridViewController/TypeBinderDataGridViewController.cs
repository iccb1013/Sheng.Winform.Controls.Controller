...
namespace Sheng.Winform.Controls.Controller
{
    ...
    public class TypeBinderDataGridViewController
    {
        ...
        /*
         * 在 T GetSelectedItem<T>() 及类似方法中
         * 对 T 的类型判断
         * 要向上判断是否兼容，如以object类型做为泛型参数，可以定位任何子元素
         * 也要向下判断是否兼容，在使用复合codon时，codon所绑定的类型是一个高级别的类型或接口
         * 而传进来的T，可能是其子类型
         * 
         * 除了判断 T 的类型是否和 codon 的类型兼容
         * 还需要判断 T 的类型是否和当前选中行绑定的对象类型兼容
         * 如 codon 是  IEntityIndex,T 是 WindowEntityIndex ，选中行绑定的对象是 FolderEntityIndex
         */
